    ...
    try {
      ...
      for(let k in parsed) {
        if(adminClientConfigs[k]) {
          ...
          adminClientConfigs[k].customMenus = parsed[k].customMenus || adminClientConfigs[k].customMenus || {};
          if (parsed[k].tiers) adminClientConfigs[k].tiers = parsed[k].tiers;
...
window.renderCustomMenus = function() {
  const clientConfig = adminClientConfigs[currentClientId];
  const container = document.getElementById("custom-menu-container");
  if (!container) return;
  
  const customKeys = Object.keys(clientConfig.customMenus || {});
  if (customKeys.length === 0) {
    container.innerHTML = '<div style="text-align:center; padding: 20px; color: var(--text-secondary); font-size: 14px;">추가된 메뉴가 없습니다.</div>';
    return;
  }
  
  let html = '<h3 style="font-size: 16px; margin-bottom: 16px; color: var(--text-primary);">추가 생성 메뉴</h3><div class="menu-grid">';
  
  customKeys.forEach(key => {
    const menu = clientConfig.customMenus[key];
    const isVisible = menu.isVisible !== false; // default true
    const typeAllChecked = menu.nameType === 'all' ? 'checked' : '';
    const typeTierChecked = menu.nameType === 'by_tier' ? 'checked' : '';
    
    html += `
      <div class="menu-row ${!isVisible ? 'disabled' : ''}" id="row-${key}" style="border: 1px solid var(--glass-border); padding: 16px; border-radius: 8px; flex-direction: column; align-items: stretch; gap: 12px; margin-bottom: 12px;">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <div class="menu-info" style="flex: 1;">
            <span class="menu-id" style="font-size: 14px; font-weight: 600; color: var(--theme-color);">${menu.id}</span>
            <div style="margin-top: 8px;">
              <label style="margin-right: 12px; font-size: 13px; cursor: pointer;"><input type="radio" name="type-${key}" value="all" ${typeAllChecked} onchange="changeCustomMenuType('${key}', 'all')" ${!isVisible ? 'disabled' : ''}> 공통 명칭</label>
              <label style="font-size: 13px; cursor: pointer;"><input type="radio" name="type-${key}" value="by_tier" ${typeTierChecked} onchange="changeCustomMenuType('${key}', 'by_tier')" ${!isVisible ? 'disabled' : ''}> 등급별 명칭</label>
            </div>
          </div>
          <div class="menu-action">
            <button class="btn" style="margin-right: 12px; padding: 6px 12px; font-size: 12px; border: 1px solid #fca5a5; color: #ef4444; background: transparent; border-radius: 6px;" onclick="deleteCustomMenu('${key}')">삭제</button>
            <span class="toggle-label" id="label-${key}">${isVisible ? '노출' : '숨김'}</span>
            <label class="switch">
              <input type="checkbox" id="toggle-${key}" ${isVisible ? 'checked' : ''} onchange="toggleCustomVisibility('${key}')">
              <span class="slider"></span>
            </label>
          </div>
        </div>
        
        <div class="menu-input-wrapper" style="width: 100%; padding-right: 0;" id="input-container-${key}">
    `;
    
    if (menu.nameType === 'all') {
      html += `<input type="text" class="form-input" id="input-${key}-all" value="${menu.defaultName || ''}" placeholder="모든 등급에 노출될 메뉴명을 입력하세요" ${!isVisible ? 'disabled' : ''}>`;
    } else {
      const tiers = clientConfig.tiers || [];
      html += `<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">`;
      tiers.forEach(tier => {
        const tName = (menu.tierNames && menu.tierNames[tier]) ? menu.tierNames[tier] : '';
        html += `
          <div style="display: flex; align-items: center; gap: 8px;">
            <span style="font-size: 12px; color: var(--text-secondary); width: 80px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">${tier}</span>
            <input type="text" class="form-input" id="input-${key}-tier-${tier}" value="${tName}" placeholder="${tier} 메뉴명" ${!isVisible ? 'disabled' : ''}>
          </div>
        `;
      });
      html += `</div>`;
...
window.addCustomMenu = function() {
  const clientConfig = adminClientConfigs[currentClientId];
  if (!clientConfig.customMenus) clientConfig.customMenus = {};
  
  const newId = 'custom_' + Math.random().toString(36).substr(2, 6);
  clientConfig.customMenus[newId] = {
    id: newId,
    nameType: 'all',
    defaultName: '새 메뉴',
    tierNames: {},
...
window.deleteCustomMenu = function(key) {
  if(confirm("이 커스텀 메뉴를 삭제하시겠습니까?")) {
    const clientConfig = adminClientConfigs[currentClientId];
    delete clientConfig.customMenus[key];
    renderCustomMenus();
  }
};

window.changeCustomMenuType = function(key, newType) {
  // Save current inputs before re-rendering
  saveCustomMenuInputs();
  const clientConfig = adminClientConfigs[currentClientId];
  if(clientConfig.customMenus[key]) {
    clientConfig.customMenus[key].nameType = newType;
  }
  renderCustomMenus();
};

window.toggleCustomVisibility = function(key) {
  const isChecked = document.getElementById(`toggle-${key}`).checked;
  const clientConfig = adminClientConfigs[currentClientId];
  if(clientConfig.customMenus[key]) {
    clientConfig.customMenus[key].isVisible = isChecked;
  }
  renderCustomMenus();
};

function saveCustomMenuInputs() {
  const clientConfig = adminClientConfigs[currentClientId];
  Object.keys(clientConfig.customMenus || {}).forEach(key => {
    const menu = clientConfig.customMenus[key];
    const isVisible = menu.isVisible !== false;
    
    if (!isVisible) return; // Ignore if hidden, though we could still save
    
    if (menu.nameType === 'all') {
       const inp = document.getElementById(`input-${key}-all`);
       if(inp) menu.defaultName = inp.value;
    } else {
       if(!menu.tierNames) menu.tierNames = {};
       const tiers = clientConfig.tiers || [];
       tiers.forEach(tier => {
         const inp = document.getElementById(`input-${key}-tier-${tier}`);
         if(inp) menu.tierNames[tier] = inp.value;
       });
    }
  });
}
...
window.saveSettings = function() {
  ...
  saveCustomMenuInputs(); // Ensure latest inputs for custom menus are saved to config
