...
window.showScheduleModal = function(hospitalName) {
  ...
  const getStatus = (day) => {
    if (window.getCheckupDateStatus) {
      return window.getCheckupDateStatus(currentYear, currentMonth, day);
    }
  ...
  const updateNavButtons = () => {
    ...
    if (prevBtn) {
      if (currentYear === 2026 && currentMonth <= 6) prevBtn.style.visibility = 'hidden';
      else prevBtn.style.visibility = 'visible';
    }
    if (nextBtn) {
      if (currentYear === 2026 && currentMonth >= 12) nextBtn.style.visibility = 'hidden';
  ...
  const renderCalendar = () => {
    ...
    for (let i = 0; i < firstDay; i++) cells += '<button class="app-cal-day other" disabled></button>';

    for (let day = 1; day <= daysInMonth; day++) {
      const dow = new Date(currentYear, currentMonth - 1, day).getDay();
      const status = getStatus(day);
      const selected = selectedDay === day ? ' selected' : '';
      const weekend = dow === 0 ? ' sunday' : (dow === 6 ? ' saturday' : '');
      const disabled = status.type === 'closed' || status.type === 'holiday';
      const clickAttr = disabled ? '' : `onclick="window.selectScheduleDay(${day})"`;
      const marker = status.type === 'partial' ? '<span class="app-cal-dot orange"></span>' : '';
      cells += `<button class="app-cal-day ${status.type}${selected}${weekend}" ${clickAttr} ${disabled ? 'disabled' : ''}>
        <span class="app-cal-num">${day}</span>${marker}
      </button>`;
    }
    return `<div class="app-calendar-grid">
      ${['일','월','화','수','목','금','토'].map((d,i)=>`<div class="app-cal-week ${i===0?'sun':i===6?'sat':''}">${d}</div>`).join('')}
      ${cells}
    </div>`;
  };
  ...
  const renderDetail = (day) => {
    const status = getStatus(day);
    const statusLabel = status.type === 'partial' ? '일부항목 마감' : (status.type === 'available' ? '예약가능' : (status.type === 'holiday' ? '휴무일' : '예약마감'));
    const closedList = status.closedItems.length ? status.closedItems.map(item => `<li>${item} 검사 마감</li>`).join('') : '<li>마감된 검사항목이 없습니다.</li>';
    const guideText = status.closedItems.length
      ? '다만, 아래 검사는 마감되었습니다. 해당 검사를 원하시면 다른 날짜를 선택해주세요.'
      : (status.type === 'available' ? '선택 가능한 검진 일정입니다.' : '해당 날짜는 예약이 마감되었습니다.');
    const btnDisabled = (status.type === 'closed' || status.type === 'holiday') ? 'disabled' : '';

    return `<div class="app-cal-detail-card">
      <h3>${currentMonth}월 ${day}일 (${['일','월','화','수','목','금','토'][new Date(currentYear, currentMonth - 1, day).getDay()]}) 상세 예약 정보</h3>
      <div class="app-cal-status-row"><span class="app-cal-status ${status.type}">${statusLabel}</span></div>
      <ul class="app-cal-closed-list">${closedList}</ul>
      <p>${guideText}</p>
      <button class="app-cal-primary-btn" ${btnDisabled} onclick="window.reserveScheduleDay(${day})">검진 일정 선택 및 예약하기</button>
    </div>`;
  };

  const modalHtml = `
    <div id="${modalId}" class="mobile-schedule-modal">
      <div class="mobile-schedule-sheet">
        <div class="mobile-schedule-header">
          <h2>${hospitalName} 예약 일정</h2>
          <button type="button" onclick="document.getElementById('${modalId}').remove()" aria-label="닫기">×</button>
        </div>
        <div class="app-calendar-card">
          <div class="app-cal-month-nav">
            <button type="button" id="${modalId}-prev" aria-label="이전 달">‹</button>
            <strong id="${modalId}-month-title">${currentYear}년 ${currentMonth}월</strong>
            <button type="button" id="${modalId}-next" aria-label="다음 달">›</button>
          </div>
          <div id="${modalId}-calendar-body">${renderCalendar()}</div>
          <div class="app-cal-legend">
            <span><i class="available"></i>예약가능</span>
            <span><i class="orange"></i>일부항목 마감</span>
            <span><i class="closed"></i>예약마감</span>
            <span><i class="holiday"></i>휴무일</span>
          </div>
        </div>
        <div id="${modalId}-detail">${renderDetail(selectedDay)}</div>
      </div>
    </div>`;
  ...
  window.selectScheduleDay = function(day) {
    selectedDay = day;
    document.getElementById(`${modalId}-calendar-body`).innerHTML = renderCalendar();
    document.getElementById(`${modalId}-detail`).innerHTML = renderDetail(day);
  };

  window.reserveScheduleDay = function(day) {
    const status = getStatus(day);
    if (status && status.closedItems && status.closedItems.length) {
      alert('선택한 날짜에는 ' + status.closedItems.join(', ') + ' 검사가 불가합니다.\n다시 선택하기를 눌러 다른 날짜를 선택해주세요.');
      return;
    }
    if (!status || status.type === 'closed' || status.type === 'holiday') {
      alert('선택한 날짜는 예약이 불가합니다.\n다시 선택하기를 눌러 다른 날짜를 선택해주세요.');
      return;
    }
    document.getElementById(modalId).remove();
    window.showToast('선택한 일정으로 예약 신청 단계로 이동합니다.');
  };

  const updateMonth = () => {
    document.getElementById(`${modalId}-month-title`).innerText = `${currentYear}년 ${currentMonth}월`;
    selectedDay = 9;
    document.getElementById(`${modalId}-calendar-body`).innerHTML = renderCalendar();
    document.getElementById(`${modalId}-detail`).innerHTML = renderDetail(selectedDay);
    updateNavButtons();
  };
  ...
  document.getElementById(`${modalId}-next`).onclick = () => {
    if (currentYear === 2026 && currentMonth >= 12) return;
    currentMonth++;
...
function renderLogin() {
  app.innerHTML = `
    <div class="mobile-shell">
      <div class="auth-wrapper fade-in" style="min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 16px; box-sizing: border-box;">
        <div class="auth-card" style="width: 100%; max-width: 100%; box-sizing: border-box;">
          <h1>통합 헬스케어 포털</h1>
...
function renderClientSelect() {
    ...
    accessibleSites.forEach(site => {
      ...
      cardsHtml += `
        ...
        ">
          ...
          <h3 style="font-size: 16px; font-weight: 700; color: #0f172a; margin-bottom: 4px;">${site.siteName}</h3>
          <span style="font-size:11px; color:${activeColor}; background:rgba(${site.themeColorRgb || '23,184,144'}, 0.1); padding:2px 8px; border-radius:10px; font-weight:600; margin-bottom:8px;">
            ${client.name}
          </span>
          <p style="font-size: 12px; color: #64748b; line-height: 1.4; flex-grow: 1; margin: 0;">
            ${(site.heroText && site.heroText.title) || '프리미엄 헬스케어 서비스'}
          </p>
  ...
  app.innerHTML = `
    <div class="mobile-shell">
      <div class="auth-wrapper fade-in" style="background: radial-gradient(circle at 50% 50%, #eff6ff 0%, #f8fafc 100%); min-height: 100vh; display: flex; align-items: center; justify-content: center; padding: 16px; box-sizing: border-box;">
        <div class="auth-card" style="width: 100%; max-width: 100%; padding: 24px; box-sizing: border-box; margin: 0;">
          <div style="text-align:center; margin-bottom:24px;">
            <h1 style="font-size: 22px; font-weight: 800; color:#0f172a; letter-spacing:-0.5px;">접속할 사이트 선택</h1>
            <p style="color:#64748b; font-size:13px; margin-top:8px;">권한이 부여된 고객사 및 맞춤형 서비스 포털 사이트를 선택해주세요.</p>
          </div>
          <div class="client-select-grid" style="display:grid; grid-template-columns: 1fr; gap:16px; width: 100%;">
            ${cardsHtml}
          </div>
          <div style="text-align:center; margin-top:32px;">
            <button class="logout-btn" style="background:#f1f5f9; color:#475569; border:1px solid #cbd5e1; font-weight:600; padding:10px 24px; border-radius:8px; width: 100%; cursor: pointer;" onclick="logout()">돌아가기 (로그아웃)</button>
          </div>
        </div>
      </div>
    </div>
...
function renderPortal() {
  ...
  // Generate organized mobile drawer menu (accordion, up to 3 depths)
  const mobileMenuHtml = visibleMenus.map((m, mIdx) => {
    const hasChildren = m.children && m.children.length > 0;
    const isActiveRoot = state.activeMenuId === m.id;
    return `
      <li class="mobile-menu-item ${isActiveRoot ? 'active' : ''}">
        <button type="button" class="mobile-menu-item-header" onclick="${hasChildren ? `toggleDrawerMenu('drawer-menu-${mIdx}')` : `window.location.hash='#/portal/${client.id}/${activeSite.siteId}/${m.id}'; toggleMobileDrawer(false);`}">
          <span class="drawer-menu-label">${m.label}</span>
          ${hasChildren ? `<span class="drawer-menu-chevron">${isActiveRoot ? '⌃' : '⌄'}</span>` : `<span class="drawer-menu-arrow">›</span>`}
        </button>
        ${hasChildren ? `
          <ul class="mobile-sub-list ${isActiveRoot ? 'open' : ''}" id="drawer-menu-${mIdx}">
            <li class="mobile-sub-item mobile-sub-home">
              <a href="#/portal/${client.id}/${activeSite.siteId}/${m.id}" onclick="toggleMobileDrawer(false);">${m.label} 홈</a>
            </li>
            ${m.children.map((sm, smIdx) => {
              const hasThird = sm.children && sm.children.length > 0;
              const isActiveSub = state.activeMenuId === m.id && state.activeSubId === sm.id;
              return `
                <li class="mobile-sub-item ${isActiveSub ? 'active' : ''}">
                  <a href="#/portal/${client.id}/${activeSite.siteId}/${m.id}/${sm.id}" onclick="toggleMobileDrawer(false);">
                    <span>${sm.label}</span><span>›</span>
                  </a>
                  ${hasThird ? `
                    <ul class="mobile-depth3-list">
                      ${sm.children.map(c3 => `
                        <li class="mobile-depth3-item ${state.activeSubSubId === c3.id ? 'active' : ''}">
                          <a href="#/portal/${client.id}/${activeSite.siteId}/${m.id}/${sm.id}/${c3.id}" onclick="toggleMobileDrawer(false);">${c3.label}</a>
                        </li>
                      `).join('')}
                    </ul>
                  ` : ''}
  ...
  if (!state.activeMenuId) {
    // DASHBOARD VIEW WITH DETAILED MOBILE TEMPLATE
    const primaryTier = userTiers[0] ? getTierName(userTiers[0]) : '';
    const service = getProvidedServiceOrDefault(client.id, primaryTier);
    const serviceTabs = service.sections.tabs || [];
    
    let serviceGuidePreviewHtml = '';
    if (serviceTabs.length > 0) {
      serviceGuidePreviewHtml = `
        <div class="home-service-guide-preview" style="margin-top: 24px; padding-top: 20px; border-top: 1px solid #e2e8f0; box-sizing: border-box; width: 100%;">
          <h4 style="font-size: 15px; font-weight: 800; color: #0f172a; margin: 0 0 12px 0; display: flex; align-items: center; gap: 6px;">
            <span>✨</span> 주요 서비스 안내
          </h4>
          <div style="display: flex; flex-direction: column; gap: 10px;">
            ${serviceTabs.map(tab => `
              <div style="padding: 12px 14px; background: #fdfdfd; border: 1px solid #f1f5f9; border-radius: 8px; box-shadow: 0 1px 2px rgba(0,0,0,0.01); box-sizing: border-box; width: 100%;">
                <div style="font-weight: 700; font-size: 13px; color: var(--theme-color); margin-bottom: 4px;">${tab.title}</div>
                <div style="font-size: 12px; color: #475569; line-height: 1.4;">${tab.content || '맞춤형 건강 증진 및 케어 필요한 프로그램'}</div>
              </div>
            `).join('')}
          </div>
        </div>
    ...
    contentHtml = `
      <div style="display: flex; flex-direction: column; width: 100%; box-sizing: border-box;">
        <!-- 1. 현재 이용 중인 서비스 -->
        <div class="current-service-card" style="padding:14px 16px; background:#eff6ff; border-radius:12px; border:1px solid #bfdbfe; margin-bottom:16px; display:flex; align-items:center; gap:12px; box-sizing:border-box; width:100%;">
          <div style="font-size:20px; display:flex; align-items:center; justify-content:center; background:#dbeafe; width:40px; height:40px; border-radius:10px; flex-shrink:0;">✨</div>
          <div>
            <div style="font-size:10px; color:#1e40af; font-weight:600; margin-bottom:2px;">현재 이용 중인 서비스</div>
            <div style="font-size:14px; font-weight:700; color:#1e3a8a; line-height:1.2;">
              ${activeSite.siteName} (${primaryTier || '일반 회원'})
            </div>
          </div>
        </div>

        <!-- 2. 메인 배너 -->
        <div class="hero-section" style="margin-bottom: 16px; box-sizing: border-box; width: 100%;">
        ...
        <!-- 4. 고객센터 -->
        <div class="cs-banner" onclick="window.location.href='tel:${activeSite.csNumber}'" style="padding: 14px 16px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; margin-bottom: 16px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; box-sizing: border-box; width: 100%;">
          ...
          <div style="font-size: 12px; color: var(--theme-color); font-weight: 700; display: flex; align-items: center; gap: 2px;">
            전화 연결 &rarr;
          </div>
        </div>
    ...
    if (isCategoryInfoPage) {
      ...
      detailContentHtml = `
        ${tabStyles}
        <div class="mobile-service-guide-page" style="display:flex; flex-direction:column; gap:18px; animation: fadeIn 0.4s ease-out;">
          ...
          <!-- 3. Tabs Area -->
          <div class="mobile-service-tabs-area" style="width:100%; margin-top: 4px;">
            <div class="mobile-section-title" style="font-size:17px; font-weight:800; color:#0f172a; margin-bottom:16px; display:flex; align-items:center; gap:8px;">
            ...
            <div class="mobile-service-tab-strip" style="display: flex; align-items: flex-end; justify-content: space-between; border-bottom: 1px solid #cbd5e1; margin-bottom: 0; width: 100%; position: relative; z-index: 2;">
      ...
      if (!state.activeSubId) {
          detailContentHtml = `          <div class="checkup-wrapper fade-in" style="padding: 20px; background: white; border-radius: 16px; border: 1px solid #cbd5e1; animation: fadeIn 0.3s ease;">
            <h3 style="font-size: 20px; font-weight: 800; color: #1e293b; margin-bottom: 12px; margin-top: 0;">건강검진 예약</h3>
            <p style="color: #334155; line-height: 1.6; font-size: 15px; margin-bottom: 20px;">
              건강검진, 나에게 맞게 직접 선택하거나 전문가와 함께 설계하세요<br/>
              고객의 건강 상태와 필요에 따라 직접 검진을 선택할 수도, 전문 상담 간호사의 도움을 받아 맞춤형 검진을 설계할 수도 있는 건강검진 서비스를 제공합니다.
            </p>
            <div style="background: #f1f5f9; padding: 16px; border-radius: 12px; margin-bottom: 24px; font-size: 13px; color: #475569; line-height: 1.6;">
              연령, 성별, 가족력, 생활습관 등을 고려한 전문 상담부터 원하는 병원과 검진 패키지를 직접 비교하고 예약하는 간편한 신청까지. 고객의 방식으로 맞는 건강검진 경험을 제공합니다.
            </div>
            <button class="auth-btn btn-primary" style="padding: 14px; font-size: 16px; width: 100%; border-radius: 12px;" onclick="window.showCheckupSupportModal()">건강검진 예약하기</button>

            <!-- Form Area: 전화상담 신청 -->
            <div id="checkup-design-form-area" style="margin-top: 40px; padding-top: 24px; border-top: 1px solid #e2e8f0; animation: fadeIn 0.4s ease;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                <h4 style="font-size: 16px; color: #1e293b; font-weight: 700; margin: 0;">전화상담 신청</h4>
                <button class="auth-btn btn-secondary" style="margin: 0; padding: 4px 8px; font-size: 11px; width: auto;" onclick="window.location.hash='#/portal/' + state.activeClient.id + '/' + state.activeSite.siteId + '/checkupAppt/checkupHistory'">상담 문의 이력</button>
              </div>
              <p style="color: #64748b; font-size: 13px; line-height: 1.5; margin-bottom: 20px; margin-top: 0;">
                평소 염려되시는 부위나 건강검진 관련 목적을 기반으로 전문 간호사의 개별 상담 전화 프로세스가 연계됩니다.
              </p>

              <div class="consult-type-selector" style="margin-bottom: 16px; display: flex; gap: 16px;">
                <label class="custom-radio">
                  <input type="radio" name="checkup-consult-type" value="phone" checked onclick="window.updateCheckupConsultType('phone')">
                  <span class="radio-mark"></span> 전화상담
                </label>
                <label class="custom-radio">
                  <input type="radio" name="checkup-consult-type" value="online" onclick="window.updateCheckupConsultType('online')">
                  <span class="radio-mark"></span> 온라인 문의
                </label>
              </div>

              <div id="checkup-guidance-box" class="consulting-guide-box" style="margin-bottom: 20px; padding: 16px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 13px; line-height: 1.5; color: #475569;">
                <i style="display: block; margin-bottom: 6px; font-style: normal; font-weight: 700; color: #1e293b;">상담 가능 시간은 평일 오전 9시 ~ 오후 6시입니다.</i>
                <i style="display: block; font-style: normal;">상담을 원하시는 날짜와 시간대를 선택하시고, 상담받고 싶은 내용을 간단히 작성해 주세요. 남겨주신 내용을 확인한 후 간호사가 직접 전화드리겠습니다.</i>
              </div>
              ...
              <div style="display: grid; grid-template-columns: 1fr; gap: 16px; margin-bottom: 16px;">
                <div class="form-group">
                  <label class="form-label" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 14px;">상담 고객 성함</label>
                  <input type="text" id="checkup-name" class="form-input" value="${state.currentUser.name}" readonly style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; background: #f1f5f9;">
                </div>
                <div class="form-group">
                  <label class="form-label" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 14px;">연락처 (핸드폰)</label>
                  <input type="tel" id="checkup-phone" class="form-input" placeholder="상담을 수신할 연락처를 입력해 주세요." style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px;">
                </div>
              </div>

              <div id="checkup-phone-fields" style="display: grid; grid-template-columns: 1fr; gap: 16px; margin-bottom: 16px;">
                <div class="form-group">
                  <label class="form-label" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 14px;">상담 희망 일자</label>
                  <input type="date" id="checkup-date" class="form-input" min="${new Date().toISOString().split('T')[0]}" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px;">
                </div>
                <div class="form-group">
                  <label class="form-label" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 14px;">상담 희망 시간</label>
                  <select id="checkup-time" class="form-input" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; background: white;">
                    <option value="anytime">전일 (언제든 통화 가능)</option>
                    <option value="09:00">09:00</option><option value="10:00">10:00</option><option value="11:00">11:00</option><option value="12:00">12:00</option>
                    <option value="13:00">13:00</option><option value="14:00">14:00</option><option value="15:00">15:00</option><option value="16:00">16:00</option><option value="17:00">17:00</option>
                  </select>
              ...
              <div class="form-group" style="margin-bottom: 20px;">
                <label class="form-label" style="display: block; margin-bottom: 6px; font-weight: 600; font-size: 14px;">염려되시는 건강 우려사항 / 가족력 / 이전 질환 등</label>
                <textarea id="checkup-memo" class="form-input" placeholder="예) 최근 혈압이 높아져 심혈관 정밀검사가 필요합니다 / 암 가족력이 있습니다" style="width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; min-height: 100px; resize: vertical; font-size: 14px;"></textarea>
              </div>

              <button onclick="submitCheckupDesign()" class="auth-btn btn-primary" style="width: 100%; padding: 14px; border-radius: 12px; font-size: 16px;">신청하기</button>
            </div>
          </div>
        ...
        detailContentHtml = `
          <div class="date-wrapper fade-in" style="animation: fadeIn 0.4s ease; max-width: 900px; margin: 0 auto; padding: 40px 0;">
            ...
            <div style="display: flex; gap: 12px;">
              <button onclick="window.history.back()" style="flex: 1; padding: 16px; border-radius: 8px; border: 1px solid #e2e8f0; background: white; color: #475569; font-size: 16px; font-weight: 700; cursor: pointer;">이전 단계</button>
              <button onclick="return window.validateCheckupWishDates();" style="flex: 1; padding: 16px; border-radius: 8px; border: none; background: #2563eb; color: white; font-size: 16px; font-weight: 700; cursor: pointer; transition: background 0.2s;">다음 단계</button>
            </div>
          </div>
        `;
    ...
    <div class="mobile-shell">
      ...
      <header class="header mobile-header">
        <div class="container header-content" style="display: flex; justify-content: space-between; align-items: center; width: 100%; height: 56px; padding: 0 16px; box-sizing: border-box;">
          <div class="logo-area" onclick="window.location.hash='#/portal/${client.id}/${activeSite.siteId}'" style="cursor:pointer; display:flex; align-items:center;">
            ${activeSite.logoImage ?
              `<img src="${activeSite.logoImage}" style="height:24px; width:auto; object-fit:contain;">` :
              `<span style="font-weight:800; font-size:18px; color:var(--theme-color);">${activeSite.serviceName || client.name}</span>`
            }
          </div>
          
          <!-- Hamburger Button -->
          <button class="mobile-hamburger-btn" onclick="toggleMobileDrawer(true)" style="background: none; border: none; cursor: pointer; padding: 4px; display: flex; align-items: center; color: #1e293b;">
      ...
      <div class="mobile-drawer" id="mobile-drawer">
        <div class="mobile-drawer-header" style="display: flex; justify-content: space-between; align-items: center; padding: 16px; border-bottom: 1px solid #f1f5f9; box-sizing: border-box; width: 100%;">
          <div class="logo-area">
            ${activeSite.logoImage ?
              `<img src="${activeSite.logoImage}" style="height:20px; width:auto; object-fit:contain;">` :
              `<span style="font-weight:800; font-size:16px; color:var(--theme-color);">${activeSite.serviceName || client.name}</span>`
            }
          </div>
          <button class="mobile-drawer-close" onclick="toggleMobileDrawer(false)" style="background: none; border: none; cursor: pointer; color: #1e293b; padding: 4px; display: flex; align-items: center;">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="drawer-profile-section">
          <div class="drawer-profile-left">
            ...
            <div class="drawer-profile-info">
              ...
              <span class="user-welcome">반갑습니다.</span>
            </div>
          </div>
          <button class="drawer-myinfo-btn" onclick="window.location.hash='#/portal/${client.id}/${activeSite.siteId}/mypage'; toggleMobileDrawer(false);">
        ...
        <div class="drawer-tab-container">
          <button class="drawer-tab-btn active" id="drawer-tab-menu" onclick="switchDrawerTab('menu')">
          ...
          <button class="drawer-tab-btn" id="drawer-tab-mypage" onclick="switchDrawerTab('mypage')">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-right:4px;">
              <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
              <circle cx="12" cy="7" r="4"></circle>
            </svg>
        ...
        <div class="mobile-drawer-content" id="drawer-menu-content-area">
        ...
        <div class="drawer-footer-links">
          <a href="#/select" onclick="toggleMobileDrawer(false);" class="drawer-link-item" style="color: var(--theme-color); font-weight: 800; border-bottom: 1px dashed rgba(var(--theme-color-rgb), 0.2); padding-bottom: 12px; margin-bottom: 8px;">
            <span>접속 사이트 변경</span>
            <svg class="drawer-link-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
          </a>
          <a href="${activeSite.clientLink || '#'}" target="_blank" class="drawer-link-item">
            <span>${activeSite.name}</span>
            <svg class="drawer-link-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
          </a>
          <a href="${activeSite.providerLink || '#'}" target="_blank" class="drawer-link-item">
            <span>${activeSite.providerName || '교보다솜케어'}</span>
            <svg class="drawer-link-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
          </a>
          <a href="#" class="drawer-link-item">
            <span>개인정보처리방침</span>
            <svg class="drawer-link-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="9 18 15 12 9 6"/></svg>
          </a>
          <a href="#" class="drawer-link-item">
        ...
        <div class="drawer-cs-box" onclick="window.location.href='tel:${activeSite.csNumber}'">
          <div class="drawer-cs-icon">
        ...
        <button class="drawer-logout-btn" onclick="logout(); toggleMobileDrawer(false);">
      ...
      <!-- Main Content Area -->
      <main class="dashboard container mobile-page ${state.activeMenuId === 'serviceGuide' ? 'mobile-page-service-guide' : ''}" style="flex:1;">${contentHtml}</main>
      ...
      <nav class="mobile-bottom-nav">
        <a href="#/portal/${client.id}/${activeSite.siteId}" class="mobile-bottom-item ${!state.activeMenuId ? 'active' : ''}">
          <span class="mobile-bottom-icon">🏠</span>
          <span>홈</span>
        </a>
        <a href="#/portal/${client.id}/${activeSite.siteId}/healthConsulting" class="mobile-bottom-item ${state.activeMenuId === 'healthConsulting' ? 'active' : ''}">
          <span class="mobile-bottom-icon">💬</span>
          <span>건강상담</span>
        </a>
        <a href="tel:${activeSite.csNumber || '1588-7524'}" class="mobile-bottom-item center-call-btn">
          <span class="mobile-bottom-icon">📞</span>
          <span>전화상담</span>
        </a>
        <a href="#/portal/${client.id}/${activeSite.siteId}/serviceGuide" class="mobile-bottom-item ${state.activeMenuId === 'serviceGuide' ? 'active' : ''}">
          <span class="mobile-bottom-icon">📋</span>
          <span>서비스안내</span>
        </a>
        <a href="#/portal/${client.id}/${activeSite.siteId}/mypage" class="mobile-bottom-item ${state.activeMenuId === 'mypage' ? 'active' : ''}">
          <span class="mobile-bottom-icon">👤</span>
          <span>마이페이지</span>
        </a>
      </nav>
...
window.toggleWishDate = function(y, m, d) {
  const status = window.getCheckupDateStatus ? window.getCheckupDateStatus(d) : { type: 'closed' };
  if (!status || status.type === 'closed' || status.type === 'holiday') {
    alert('선택한 날짜는 예약이 불가합니다.\n다른 날짜를 선택해주세요.');
    return;
  }
...
window.validateCheckupWishDates = function() {
  try {
    const selected = Array.isArray(window.selectedWishDates) ? window.selectedWishDates : [];
    if (selected.length < 2) {
      if (window.showToast) window.showToast('검진희망일 2개를 모두 선택해주세요.');
      else alert('검진희망일 2개를 모두 선택해주세요.');
      return false;
    }

    const blocked = [];
    selected.forEach(function(dateStr) {
      const parts = String(dateStr).split('-');
      const year = parseInt(parts[0], 10);
      const month = parseInt(parts[1], 10);
      const day = parseInt(parts[2], 10);
      const status = window.getCheckupDateStatus ? window.getCheckupDateStatus(year, month, day) : null;
      if (status && Array.isArray(status.closedItems) && status.closedItems.length) {
        status.closedItems.forEach(function(item) {
          if (blocked.indexOf(item) === -1) blocked.push(item);
        });
      }
    });

    if (blocked.length > 0) {
      alert(blocked.join(', ') + ' 항목이 마감되어 예약이 불가합니다.\n다른 날짜를 선택해주세요.');
      return false;
    }

    const routeParts = String(state.route || window.location.hash || '').split('/');
    const clientId = (state.activeClient && state.activeClient.id) || routeParts[2] || 'kyobo';
    const siteId = (state.activeSite && state.activeSite.siteId) || routeParts[3] || 'default';
    const menuId = state.activeMenuId || routeParts[4] || 'checkupAppt';

    window.location.hash = '#/portal/' + clientId + '/' + siteId + '/' + menuId + '/checkup-summary';
    return false;
  } catch (error) {
    console.error('validateCheckupWishDates error:', error);
    alert('다음 단계 이동 중 오류가 발생했습니다.\n선택한 날짜를 다시 확인해주세요.');
    return false;
  }
};
...
window.renderStep4Calendar = function() {
  ...
  const selected = window.selectedWishDates || [];

  const prevStyle = (y === 2026 && m <= 6) ? 'style="visibility: hidden;"' : '';
  const nextStyle = (y === 2026 && m >= 12) ? 'style="visibility: hidden;"' : '';

  let html = '<div class="app-calendar-card checkup-request-calendar">';
  html += '<div class="app-cal-month-nav">' +
          '<button type="button" onclick="window.prevStep4Month()" aria-label="이전 달" ' + prevStyle + '>‹</button>' +
          '<strong>' + y + '년 ' + m + '월</strong>' +
          '<button type="button" onclick="window.nextStep4Month()" aria-label="다음 달" ' + nextStyle + '>›</button>' +
          '</div>';
  html += '<div class="app-calendar-grid">';
  ['일','월','화','수','목','금','토'].forEach(function(dayName, i) {
    html += '<div class="app-cal-week ' + (i===0?'sun':i===6?'sat':'') + '">' + dayName + '</div>';
  });

  for (let i = 0; i < firstDay; i++) html += '<button class="app-cal-day other" disabled></button>';
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    const disabled = status.type === 'closed' || status.type === 'holiday';
    const clickAttr = disabled ? '' : 'onclick="window.toggleWishDate('+y+', '+m+', '+d+')"';
    const marker = status.type === 'partial' ? '<span class="app-cal-dot orange"></span>' : '';
    html += '<button class="app-cal-day ' + status.type + (isSelected ? ' selected' : '') + weekend + '" ' + clickAttr + (disabled ? ' disabled' : '') + '><span class="app-cal-num">' + d + '</span>' + marker + '</button>';
  }
  html += '</div>';
  html += '<div class="app-cal-legend"><span><i class="available"></i>예약가능</span><span><i class="orange"></i>일부항목 마감</span><span><i class="closed"></i>예약마감</span><span><i class="holiday"></i>휴무일</span></div>';

  const selectedDaysInfo = selected.map(function(dateStr) {
  ...
  let partialItems = [];
  selectedDaysInfo.forEach(function(info) {
    const status = window.getCheckupDateStatus(info.year, info.month, info.day);
    if (status.closedItems && status.closedItems.length) {
      status.closedItems.forEach(function(item) {
        if (!partialItems.includes(item)) partialItems.push(item);
      });
    }
  });

  if (selected.length > 0) {
    html += '<div class="checkup-selected-panel">';
    html += '<div class="checkup-selected-title">선택한 날짜</div>';
    html += '<div class="checkup-selected-dates">' + selected.map(function(dateStr, idx) { return '<span>' + (idx + 1) + '순위 ' + dateStr.replace(/-/g, '.') + '</span>'; }).join('') + '</div>';
    if (partialItems.length > 0) {
      html += '<div class="checkup-closed-notice"><strong>선택한 날짜에 마감된 검사항목이 있습니다.</strong><ul>' + partialItems.map(function(item) { return '<li>' + item + ' 검사 마감</li>'; }).join('') + '</ul><p>해당 항목을 예약하려면 다른 날짜를 선택해주세요.</p></div>';
    } else {
      html += '<div class="checkup-open-notice">선택한 날짜는 마감된 검사항목 없이 예약 가능합니다.</div>';
    }
    html += '</div>';
  } else {
    html += '<div class="checkup-selected-panel empty">예약 가능한 날짜를 2개 선택해주세요.</div>';
  }
  html += '</div>';
...
window.switchDrawerTab = function(tab) {
  const menuBtn = document.getElementById('drawer-tab-menu');
  const mypageBtn = document.getElementById('drawer-tab-mypage');
  const contentArea = document.getElementById('drawer-menu-content-area');
  if (!contentArea) return;

  if (tab === 'menu') {
    if (menuBtn) menuBtn.classList.add('active');
    if (mypageBtn) mypageBtn.classList.remove('active');
    
    const client = state.activeClient;
    const activeSite = state.activeSite || (client ? client.sites[0] : null);
    const userTiers = state.currentUser ? (state.currentUser.tiers[client.id] || []) : [];
    if (client && activeSite) {
      const visibleMenus = filterMenusForUser(activeSite.menus || [], userTiers);
      const menuHtml = visibleMenus.map(m => {
        const hasChildren = m.children && m.children.length > 0;
        return `
          <li class="mobile-menu-item" style="border-bottom: 1px solid #f1f5f9;">
            <div class="mobile-menu-item-header" style="padding: 14px 20px; font-weight: 700; font-size: 15px; color: #1e293b; display: flex; justify-content: space-between; align-items: center; cursor: pointer;" onclick="window.location.hash='#/portal/${client.id}/${activeSite.siteId}/${m.id}'; toggleMobileDrawer(false);">
              <span>${m.label}</span>
              ${hasChildren ? `<span style="font-size:12px; color:#94a3b8;">▼</span>` : ''}
            </div>
            ${hasChildren ? `
              <ul class="mobile-sub-list" style="background: #f8fafc; padding: 6px 0; border-top: 1px solid #f1f5f9;">
                ${m.children.map(sm => `
                  <li class="mobile-sub-item" style="padding: 8px 32px; font-size: 14px;">
                    <a href="#/portal/${client.id}/${activeSite.siteId}/${m.id}/${sm.id}" style="color: #475569; text-decoration: none; display: block; font-weight: 500;" onclick="toggleMobileDrawer(false);">${sm.label}</a>
                  </li>
                `).join('')}
              </ul>
            ` : ''}
      ...
      contentArea.innerHTML = `<ul class="mobile-menu-list">${menuHtml}</ul>`;
    }
  } else if (tab === 'mypage') {
    ...
    if (client && activeSite) {
      const mypageHtml = `
        <li style="border-bottom: 1px solid #f1f5f9;">
          <a href="#/portal/${client.id}/${activeSite.siteId}/mypage" style="display:block; padding: 14px 20px; font-weight: 700; font-size: 15px; color: #1e293b; text-decoration:none;" onclick="toggleMobileDrawer(false);">👤 마이페이지 홈</a>
        </li>
        <li style="border-bottom: 1px solid #f1f5f9;">
          <a href="#/portal/${client.id}/${activeSite.siteId}/mypage" style="display:block; padding: 14px 20px; font-weight: 500; font-size: 14px; color: #475569; text-decoration:none;" onclick="toggleMobileDrawer(false);">예약 및 신청 내역</a>
        </li>
        <li style="border-bottom: 1px solid #f1f5f9;">
          <a href="#/portal/${client.id}/${activeSite.siteId}/mypage" style="display:block; padding: 14px 20px; font-weight: 500; font-size: 14px; color: #475569; text-decoration:none;" onclick="toggleMobileDrawer(false);">구독 콘텐츠 관리</a>
        </li>
      `;
      contentArea.innerHTML = `<ul class="mobile-menu-list">${mypageHtml}</ul>`;
    }
  }
};
...
window.toggleDrawerMenu = function(id) {
  const target = document.getElementById(id);
  if (!target) return;
  const isOpen = target.classList.contains('open');
  document.querySelectorAll('.mobile-sub-list.open').forEach(el => {
    if (el.id !== id) el.classList.remove('open');
  });
  target.classList.toggle('open', !isOpen);
};

/* PC-same Mobile UX helper: does not replace PC content, only adds route classes and touch-friendly states. */
(function () {
  function markMobileRoute() {
    try {
      var body = document.body;
      if (!body) return;
      var hash = window.location.hash || '';
      body.classList.toggle('route-checkup', hash.indexOf('/checkupAppt') > -1);
      body.classList.toggle('route-checkup-flow', /checkup-(target|package|option|date|summary)|consent/.test(hash));
      body.classList.toggle('route-service-guide', hash.indexOf('/serviceGuide') > -1);
      body.classList.toggle('route-home', /#\/portal\/[^/]+\/[^/]+$/.test(hash));
      document.querySelectorAll('.responsive-table-wrapper').forEach(function (el) {
        if (!el.dataset.scrollHintAdded) {
          el.dataset.scrollHintAdded = '1';
          el.setAttribute('aria-label', '좌우로 스크롤하여 표 내용을 확인하세요');
        }
      });
    } catch (e) {}
  }
  window.addEventListener('hashchange', function(){ setTimeout(markMobileRoute, 80); });
  window.addEventListener('DOMContentLoaded', function(){ setTimeout(markMobileRoute, 120); });
  var mo = new MutationObserver(function(){ markMobileRoute(); });
  window.addEventListener('DOMContentLoaded', function(){ if (document.body) mo.observe(document.body, {childList:true, subtree:true}); });
})();
